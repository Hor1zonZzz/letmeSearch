...
- 获取最多三个指定 URL 的正文；
- 将 Exa 返回的页面图片链接在本机做后台归档；
- 按原始研究问题压缩单个长页面；
- 由 Agent 迭代调用工具、综合答案并选择最终引用；
- 通过 Flue Workflow 暴露结构化输入与输出。

当前明确不承担或尚未实现的能力包括：全网爬取与站点递归、登录态页面、对附件或多媒体的专项解析与效果保证、搜索索引自建、事实正确性的确定性证明、来源自动评级、引用到原文段落的定位，以及跨请求缓存或研究状态复用。Exa 可能自行支持部分 PDF 等内容，但这属于供应商能力，不是本系统当前定义和验证的契约。系统也不是通用网页代理：工具仅提供搜索与内容获取，没有浏览器交互能力。
...
flowchart LR
    ...
    F -->|仅 URLs + 图片链接请求| E[Exa Contents API]
    F -->|长页面 + query| C[DeepSeek 直连压缩器]
    F -->|后台下载与去重| R[data/resources 图片归档]
    G --> A
    E --> F
    C --> F
    F --> A
...
1. 仅把 URL 列表发送给 Exa Contents，单页请求正文上限为 50,000 字符，
   并以接口当前允许的最大值 `extras.imageLinks=5000` 请求页面图片链接；
   研究问题**不发送给 Exa**。
2. 每个返回页面独立处理。正文不超过 6,000 字符时原样返回；更长时使用 DeepSeek `deepseek-v4-flash` 压缩。页面主图与额外图片链接同时交给后台归档模块。
...
5. 输出给出 Exa 返回的页面、根据状态列表识别出的未成功 URL 和原始正文字符数。图片归档不进入工具输出，也不提供给 Agent 或 Workflow 调用方。
...
Node 目标通过源目录中的 `db.ts` 配置 Flue 内建 SQLite adapter，运行记录写入项目根目录的 `data/flue.db`。该目录被 Git 忽略；首次使用时 adapter 会创建目录、数据库和 schema，并以 WAL 模式运行。它保存 Flue 自己的 workflow run 记录、事件流、输入、终态结果或错误，以及运行时需要的会话和附件数据，使这些记录在同一主机的进程重启后仍可读取。

这不是跨请求研究缓存：新的请求仍会重新创建 Agent session、重新搜索并使用独立的 URL 去重集合；数据库不会自动复用上一轮的搜索结果或页面内容。它也不是业务数据库，不能替代应用自己的用户、配额、审计或数据保留模型。由于 query、答案及部分运行事件可能包含敏感内容，`data/` 不得提交、公开下载或作为静态资源目录暴露。

当前 Workflow 只暴露创建 run 的 `route`，尚未导出 Flue 的 `runs` middleware，因此历史记录不会通过 HTTP 读取。未来若要开放 `GET /runs/:runId` 或管理员列表，必须先定义调用方身份、按用户的归属授权和保留/删除策略；不能因为本地 SQLite 已持久化就默认公开读取历史。

### 3.7 图片归档

`web_fetch` 将 Exa 的页面主图和 `extras.imageLinks` 合并后交给独立归档模块。图片 URL 会移除 fragment、规范化协议默认端口和主机大小写，但保留完整查询参数，再以规范化 URL 的 SHA-256 作为稳定资源 ID。相同 URL 在同一次调用、并发调用和后续进程运行中均复用同一资源；重复出现时只合并来源页面，不重复下载。

图片存放在 `IMAGE_RESOURCE_DIR`（默认 `data/resources`）下的两级哈希目录。每张图片旁有同 ID 的 JSON sidecar，保存原始 URL、规范化 URL、来源页面、本地相对路径、MIME、字节数、内容哈希、尝试次数和失败信息。图片及 sidecar 使用临时文件加原子重命名写入。它们不进入 `flue.db`；SQLite 仍只负责 Flue 运行时状态。

下载只接受 HTTP(S) 和 `image/*` 响应，拒绝显式私网、本机及不可路由地址，并逐次检查重定向目标。单图默认限制为 10 MiB，可通过 `IMAGE_MAX_BYTES` 调整；超时为 15 秒，最多跟随 5 次重定向，并发数为 4。下载和写盘是尽力而为：单图失败会写入失败 sidecar，不影响正文研究；调用取消仍会向上传播。归档任务会在 `web_fetch` 返回前收敛，因此它不是脱离进程的持久任务队列，图片较多时会增加工具延迟。

当前不提供图片 HTTP 路由、图片检索 API、清理策略或容量配额。`data/resources` 不能视为永久对象存储；单机磁盘丢失时图片不会由 SQLite 恢复。
...
当前预算体现“先广搜、后精选、逐页限长”：每次搜索至多 10 个主结果；每次抓取至多 3 页；每页最多读取 50,000 字符；交给 Agent 的单页内容通常至多 6,000 字符；压缩生成上限为 2,000 tokens。短页不额外调用模型，避免无收益的摘要成本与信息损失。图片链接按 Exa 当前接口上限每页最多请求 5,000 条，实际归档数量取决于页面返回结果；图片下载并发为 4，单图默认最多 10 MiB，但当前没有任务级图片总字节预算。
...
- 单张图片下载、格式校验或写盘失败只更新失败 sidecar，不改变 `web_fetch` 的正文输出；非取消的归档级异常同样不会中断研究。

`raw`、`compressed` 和 `fallback_excerpt` 是重要的证据质量信号，但 Agent 指令没有明确规定如何按这些信号降低结论强度。这是后续应优先补齐的策略缺口。当前输出也无法判断 Exa 是否已在 50,000 字符处截断，因此不能把一次成功获取解释为“页面完整”。
...
| 图片按 URL 哈希落盘并配 sidecar | 跨运行去重，不污染 Agent 上下文和 Flue 数据库 | 增加抓取延迟与本机磁盘占用，尚无清理策略 |
...
10. **图片归档容量**：单页可返回大量图片链接，虽然单图和并发受限，但没有 Run 级数量、总字节、保留期或磁盘水位限制。远程服务器也可能在 DNS 校验后发生解析变化，当前的私网地址检查不能替代网络层出站策略。
