...
DEEPSEEK_API_KEY=...
TWITTERAPI_IO_KEY=...
# shadow 只比较新旧 Resolver；write 正式提交，默认 write
NEWS_TOPIC_RESOLVER_MODE=write
```
...
上线切换可先设置 `NEWS_TOPIC_RESOLVER_MODE=shadow`。Shadow 模式对同一 Pending Post
分别运行工具型 Resolver 和旧版全候选 Resolver，将决策、一致性、模型 Run、搜索轨迹和
错误写入 `topic_resolution_shadow_comparisons`，但不创建 Topic 或改写成员关系；同一
Resolver 版本已比较的 Post 不会重复消耗模型。核验一致率和差异样本后，将配置改为
`write`，Pending Post 会重新进入正式写入队列。也可对单次手工 Run 传入 Workflow
input `{"mode":"shadow"}` 或 `{"mode":"write"}` 覆盖环境配置。
