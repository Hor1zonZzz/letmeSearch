...
function configuredApiKey(): string {
	const value = process.env.TWITTERAPI_IO_KEY?.trim()
		?? process.env.TWITTERAPI_IO_API_KEY?.trim();
	if (!value) throw new Error('Missing required environment variable: TWITTERAPI_IO_KEY');
	return value;
}
...
export class TwitterApiClient {
	...
	async fetchLatestTweets(
		...
		if (!response.ok) {
			throw new Error(`TwitterAPI.io request failed (${response.status}): ${text.slice(0, 500)}`);
		}
		...
		if (typeof root.status === 'string' && root.status !== 'success') {
			throw new Error(`TwitterAPI.io returned an error: ${String(root.msg ?? root.message ?? root.status).slice(0, 500)}`);
		}
		...
		return {
			tweets: rawTweets.slice(0, limit),
			hasNextPage: root.has_next_page === true,
			nextCursor: typeof root.next_cursor === 'string' && root.next_cursor.length > 0
				? root.next_cursor
				: null,
		};
	}
