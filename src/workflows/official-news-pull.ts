...
import { classifyPostsByAccount } from "../news/post-classifier";
import { reportPrompt } from "../news/prompts";
import {
	renderReportMarkdown,
	reportPathForEvent,
	writeReportFile,
} from "../news/report-files";
import { reportDraftSchema } from "../news/schemas";
...
const inputSchema = v.object({
	maxPostsPerAccount: v.optional(
		v.pipe(
			...
			v.integer(),
			v.minValue(1),
			v.maxValue(MAX_POSTS_PER_ACCOUNT),
		),
		DEFAULT_POSTS_PER_ACCOUNT,
	),
});
...
async function processImportantPost(options: {
	...
	if (materialChange) {
		const previousMarkdown = existing
			? database.getCurrentReportMarkdown(
					existing.id,
					existing.currentReportVersion,
				)
			: null;
...
async function exportPendingReports(
	database: NewsDatabase,
	stats: WorkflowStats,
): Promise<void> {
	for (const report of database.listPendingReportExports()) {
		try {
			...
			stats.errors.push({
				scope: `report-file:${report.eventId}:${report.version}`,
				message,
			});
		}
	}
		...
		try {
				...
				try {
					...
					for (const rawTweet of response.tweets) {
						try {
							...
							if (result.isNew) {
								stats.newPosts += 1;
								if (result.post.processingStatus === "ignored")
					...
					database.recordAccountPullSuccess(
						account.id,
						profile,
						lastSeenPostAt,
						new Date().toISOString(),
			...
			if (posts.length > 0) {
				...
				for (const post of posts) {
					const analysis = classification.analyses.get(post.id);
					if (!analysis) continue;
					stats.analyzedPosts += 1;
					if (!analysis.isImportant) {
						database.markPostIgnored(
							post.id,
							analysis,
							ANALYSIS_VERSION,
							new Date().toISOString(),
					...
					if (!isImportantAnalysis(officialAnalysis)) {
						const message =
							"Important classification omitted required event fields or facts";
						database.markPostFailed(post.id, message, new Date().toISOString());
